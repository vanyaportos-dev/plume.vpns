...
async function loadPlans() {
  ...
  try {
    ...
    if (!d.plans || !d.plans.length) {
      el.innerHTML = '<p style="color:var(--text-secondary);">Нет доступных тарифов</p>';
      return;
    }
    ...
    el.innerHTML = '<p style="color:var(--danger);">Ошибка загрузки тарифов</p>';
  }
}
...
async function buyPlan(planUuid) {
  ...
  try {
    const r = await fetch('/api/vpn/subscribe', {
      ...
      body: JSON.stringify({
        plan_uuid: planUuid,
        external_user_id: user.telegram_id || user.email
      })
    });
    const d = await r.json();
    if (d.success !== false) {
      alert('✅ Подписка создана!\n\nСсылка: ' + (d.subscription_url || 'скоро'));
      loadSubscriptions();
    } else {
...
async function loadSubscriptions() {
  ...
  el.innerHTML = '<p style="color:var(--text-secondary);">Загрузка подписок...</p>';
  // Заглушка — позже добавим загрузку из Redis
  setTimeout(() => {
...
async function loadDevices() {
  ...
  el.innerHTML = '<p style="color:var(--text-secondary);">Загрузка устройств...</p>';
  setTimeout(() => {
